...
app.get('/api/superadmin/total-queries',
  ...
  async (req, res) => {
    try {
        ...
        // console.log(analyticsData);   //Comment out this line to see the queries and other details in console
...
app.post('/api/knowledge-requests', 
  ...
  async (req, res) => {
    ...
    // Create new path with numbered filename
    const newPath = path.join(uploadDir, newFilename);
    
    // Rename the file to preserve both versions
    fs.renameSync(req.file.path, newPath);
    
    filePath = newPath;
    content = `PDF:${newFilename}`; // Store numbered name in database
    
    console.log(`PDF stored as: ${newFilename}`); 
    } 
