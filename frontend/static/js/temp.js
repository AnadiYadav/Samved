...
document.getElementById('informationForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (contentType === 'link') {
            ...
            console.log(`• HTML Pages: ${all_links.length}`);
            console.log(`• PDF Documents: ${pdf_links.length}`);

            // Process HTML links
            if (all_links.length > 0) {
                console.log(`\n🚀 PROCESSING HTML PAGES`);
                for (const [index, link] of all_links.entries()) {
                    const currentCount = index + 1;
                    try {
                        console.log(`\n📄 [HTML ${currentCount}/${all_links.length}] STARTING: ${link}`);
                        ...
                        successCount++;
                        console.log(`✅ [HTML ${currentCount}/${all_links.length}] SUCCESS: ${link}`);
                    } catch (error) {
                        errorCount++;
                        console.error(`❌ [HTML ${currentCount}/${all_links.length}] FAILED: ${link}\nREASON: ${error.message}`);
                    }
                    console.log(`📈 PROGRESS: ${successCount + errorCount}/${totalLinks} (${Math.round(((successCount + errorCount)/totalLinks)*100)}%)`);
                }
            }
            ...
            if (pdf_links.length > 0) {
                ...
                for (const [index, link] of pdf_links.entries()) {
                    const currentCount = index + 1;
                    const overallCount = all_links.length + index + 1;
                    try {
                        console.log(`\n📑 [PDF ${currentCount}/${pdf_links.length}] STARTING: ${link}`);
                        ...
                        successCount++;
                        console.log(`✅ [PDF ${currentCount}/${pdf_links.length}] SUCCESS: ${link}`);
                    } catch (error) {
                        errorCount++;
                        console.error(`❌ [PDF ${currentCount}/${pdf_links.length}] FAILED: ${link}\nREASON: ${error.message}`);
                    }
                    console.log(`📈 PROGRESS: ${overallCount}/${totalLinks} (${Math.round((overallCount/totalLinks)*100)}%)`);
                }
            }

            console.log(`\n=== PROCESSING SUMMARY ===`);
            console.log(`✅ Successful: ${successCount}/${totalLinks}`);
            console.log(`❌ Failed: ${errorCount}/${totalLinks}`);
            console.log(`⏱️  Completion Rate: ${Math.round((successCount/totalLinks)*100)}%`);
            ...
            console.log(`\n📤 UPLOADING PDF FILE: ${fileInput.files[0].name}`);
            ...
            console.log(`✅ PDF PROCESSED SUCCESSFULLY: ${fileInput.files[0].name}`);
        ...
        console.error(`\n‼️ CRITICAL PROCESSING ERROR: ${error.message}`);
